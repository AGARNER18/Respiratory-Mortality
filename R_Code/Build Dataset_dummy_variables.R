...
DATA2<-cbind(DATA2, dummy(DATA2$MARITAL_STATUS, sep="_", drop=TRUE, verbose = TRUE))
DATA2<-cbind(DATA2, dummy(DATA2$LANGUAGE, sep="_", drop=TRUE, verbose = TRUE))
DATA2<-cbind(DATA2, dummy(DATA2$ORG_NAME, sep="_", drop=TRUE, verbose = TRUE))
DATA2<-cbind(DATA2, dummy(DATA2$INSURANCE, sep="_", drop=TRUE, verbose = TRUE))
DATA2<-cbind(DATA2, dummy(DATA2$ETHNICITY, sep="_", drop=TRUE, verbose = TRUE))
DATA2<-cbind(DATA2, dummy(DATA2$ADMISSION_LOCATION, sep="_", drop=TRUE, verbose = TRUE))
DATA2<-cbind(DATA2, dummy(DATA2$GENDER, sep="_", drop=TRUE, verbose = TRUE))
DATA2$MARITAL_STATUS<-NULL
DATA2$RRT<-NULL
DATA2$LANGUAGE<-NULL
DATA2$ADMISSION_TYPE<-NULL
DATA2$ORG_NAME<-NULL
DATA2$INSURANCE<-NULL
DATA2$ETHNICITY<-NULL
DATA2$ADMISSION_LOCATION<-NULL
...
training <- SMOTE(target ~ ., training, perc.over = 200)
table(training$target)
prop.table(table(training$target))
...
write.csv(pilot, "pilot_dummy_vars.csv")
write.csv(training, "train_dummy_vars.csv")
write.csv(test, "test_dummy_vars.csv")
